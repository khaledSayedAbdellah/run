...
class _SearchAddressViewState extends State<SearchAddressView> {
  ...
  void initState() {
    super.initState();
    formLocation = widget?.placeBloc?.fromLocation?.formattedAddress;
  }
      ...
      child: Column(
        children: <Widget>[
          ...
          widget?.placeBloc?.listPlace != null
              ? Expanded(
                  child: widget.placeBloc.listPlace.isNotEmpty
                      ? ListView.separated(
                          shrinkWrap: true,
                          itemCount: widget?.placeBloc?.listPlace?.length,
                          itemBuilder: (context, index) {
                            return ListTile(
                              leading: Icon(OMIcons.locationOn),
                              title: Text(
                                  widget?.placeBloc?.listPlace[index].name),
                              subtitle: Text(widget?.placeBloc?.listPlace[index]
                                  .formattedAddress),
                              onTap: () {
                                widget?.placeBloc
                                    ?.selectLocation(
                ...
                children: <Widget>[
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: localize(context, 'from'),
                    ),
                    controller: TextEditingController.fromValue(
                      TextEditingValue(
                        text: formLocation != null ? formLocation : '',
                        selection: TextSelection.collapsed(
                            offset: formLocation != null
                                ? formLocation?.length
                                : 0),
                      ),
                    ),
...
//                                  ):SizedBox.shrink()
                    ...
                    controller: TextEditingController.fromValue(
                      TextEditingValue(
                        ...
                        selection: TextSelection.collapsed(
                            offset:
                                toLocation != null ? toLocation?.length : 0),
                      ),
                    ),
                    onChanged: (String value) async {
                      toLocation = value;
