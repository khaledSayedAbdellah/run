...
class _HomeScreen2State extends State<HomeScreen2> {
    ...
    _getCurrentLocation().then((value) {
      showNearDrivers(currentLocation.latitude, currentLocation.longitude)
        ...
        for (Driver driver in allNearDrivers) {
          print(driver.name);
          _addMarker(driver.id, double.parse(driver.driverLat), double.parse(driver.driverLong));
          setState(() {
            dataMarKer.add({
              "id": driver.id,
              "lat": double.parse(driver.driverLat),
              "lng": double.parse(driver.driverLong),
            });
          });
        }
      });
  ...
  void _onMapCreated(GoogleMapController controller) {
    ...
    for (int i = 0; i < dataMarKer.length; i++) {
      ...
      if (distance * 1000 < _radius) {
        _addMarker(
            dataMarKer[i]['id'], dataMarKer[i]['lat'], dataMarKer[i]['lng']);
      }
    }
  }
  ...
  void _addMarker(String markerIdVal, double lat, double lng) async {
    ...
    final Marker marker = Marker(
      ...
      icon: checkPlatform
          ? BitmapDescriptor.fromAsset("assets/image/marker/car_top_48.png")
          : BitmapDescriptor.fromAsset("assets/image/marker/car_top_96.png"),
      onTap: () {
